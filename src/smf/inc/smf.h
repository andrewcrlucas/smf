 ...
 * MACROS                                                                    *
 *****************************************************************************/
#define SMF_CONSTRUCTOR(a, b, c, d) (smf((p_smf_t)me, &g_state_jump_table[0], (a), (b), (c), (d)))
#define SMF_DECONSTRUCTOR() (smf_((p_smf_t)me))
#define SMF_GET_EVENT(module_name) (module_name##_get_event(me, smf_get_event((p_smf_t) me)))
...
#define SMF_RUN_SMF() smf_run((p_smf_t) me)
...
#define SMF_RESPOND_WITH_SIGNAL(thread_id, p_signals) smf_respond_to_client_with_signal((thread_id), (p_signals))
#define SMF_RESPOND_WITH_MESSAGE(p_message_queue, message) smf_respond_to_client_with_message((p_message_queue), (message))
#define SMF_RELEASE_MUTEX(mutex) smf_release_mutex((mutex));

#define SMF_EXPAND_AS_STATE_ENUM(a,b) a##_STATE,
#define SMF_EXPAND_AS_SMF_PROTOTYPES(a,b) static void b##_state(void *);

#define SMF_SMF_JUMP_TABLE_DECLARATION static void (*const g_state_jump_table[]) (void *) =

#define SMF_EXPAND_AS_SMF_JUMP_TABLE_ENTRIES(a,b) b##_state,
#define SMF_SMF_CREATE_JUMP_TABLE(STATE_TABLE)            \
    static void (*const g_state_jump_table[]) (void *) =  \
    {                                                     \
        STATE_TABLE(SMF_EXPAND_AS_SMF_JUMP_TABLE_ENTRIES) \
    };
...
#define SMF_SMF_IS_SIGNAL_SET(signal, requested_signal_mask) smf_is_signal_set(signal, requested_signal_mask)
